            ...
            <div key={game.id} className="card px-4 py-3 animate-card-enter" style={{ animationDelay: `${i * 60}ms` }}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5 flex-wrap min-w-0">
                  <span className={`font-semibold text-sm ${game.winnerId === game.player1Id ? 'text-accent' : game.loserId === game.player1Id ? 'text-negative' : ''}`}>
                    {p1Name}
                  </span>
                  ...
                  <span className={`font-semibold text-sm ${game.winnerId === game.player2Id ? 'text-accent' : game.loserId === game.player2Id ? 'text-negative' : ''}`}>
                    {p2Name}
                  </span>
                </div>
              ...
              {!isTie && loserName && game.drinksOwed > 0 && (
                <div className="flex items-center gap-2 mt-1">
                  ...
                  {allFulfilled && (
                    <span className="text-highlight text-[10px] font-semibold">Done</span>
                  )}
                </div>
              )}
              {isTie && (
